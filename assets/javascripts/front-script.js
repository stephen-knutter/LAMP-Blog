...
$(function(){
	...
	var listClick = false;
	//DIRECTIONS
	$("#directionsIcon").on("click", function(event){
		event.preventDefault();
		$("#directions").animate({
			height:["toggle", "swing"],
			width: ["toggle", "swing"]
		...
		var icon = listClick ? "https://www.budvibes/images/expand-icon.png" : "https://www.budvibes.com/images/collapse-icon.png"
		$("#directionsIcon img").attr("src", icon);
		listClick = !listClick
	});
	...
	$(document).on("click",function(event){
		...
		if(!clickedForum && !clickedLib && !clickedMap && !clickedLog && !clickedLog && !clickedList && 
			(!clickedForumIcon && !clickedLibIcon && !clickedMapIcon && !clickedLogIcon && !clickedListIcon) && ($i > 1) && openItem){
...
/*SWITCH BETWEEN LOG IN & SIGN UP FORMS*/
$(function(){
	/*
	$("body").on("click", "a.#signInLink", function(event){
		event.preventDefault();
		$(".signUp").css({display: "none", visibility: "hidden"});
		$(".logIn").css({display: "block", visibility: "visible"});
	});
	
	$("body").on("click", "a.#signUpLink", function(event){
		event.preventDefault();
		$(".logIn").css({display: "none", visibility: "hidden"});
		$(".signUp").css({display: "block", visibility: "visible"});
	})
	*/
	/*
	referer = {
	...
	referer.setSignUpForm = function(){
		this.curParent = this.curElem.closest("#logMenu");
		this.curUri = this.curParent.find("#logWrap form #back_uri").val();
		this.curParent.html(
			'<h3><img src="../../images/sign-up-head.png" alt="User Sign Up"></h3>'+
			'<div id="logWrap">'+
	...
	referer.setLogInForm = function(){
		this.curParent = this.curElem.closest("#logMenu");
		this.curParent.html(
				'<h3><img src="../../images/log-in-head.png" alt="Log In"></h3>'+
				'<div id="logWrap">'+
				'<form action="../login.php" method="post" id="signUpForm">'+
				'<input type="hidden" id="back_uri" name="back_uri" value="'+this.curUri+'">'+
				'<input type="text" class="loginInput" name="email" id="loginText" placeholder="Email">'+
				'<input type="password" class="loginInput" name="password" id="loginPass" placeholder="Password">'+
				'<input type="submit" class="loginSubmit" id="submitLogin" value="Log In">'+
				'</form>'+
				'<div id="signUp">'+
	...
	$("body").on("click", "a.#signUpLink", function(event){
		event.preventDefault();
		referer.curElem = $(this);
		referer.setSignUpForm();
	});
	...
	setInterval(function(){
		...
		if(didScroll && iscriptType != 'none'){
			...
			if(iscrollTop >= idocHeight - istartAjax){
				...
				url = 'https://www.budvibes.com/ajax-front.php?"'+iTime+'"';
				...
				if(!gifStop){
					$.ajax({
						...
						type: 'POST',
						cache: 'false',
						data: {user: iuser, start: istart, type: type},
						url: url,
								...
								$.each(photoBatch, function(i,v){
									...
									if(j%2){
										if(newPhoto.vidtype == 'user'){
											leftPane.append(
												"<div class='userPicWrap'>"+
													'<video loop style="margin: 0 auto; position: relative; display: block;" id="video-preview'+timeStamp+'" class="video-js vjs-default-skin" controls preload="auto" width="281" height="281">'+
												 	'<source src="'+newPhoto.photo+'">'+
...
$(function(){
		...
		var radioChange = $("ul#typeFilter li input[type=radio]");
		$(radio).each(function(){
		...
		$(radioChange).click(function(){
			radioChange.prop("checked", false).parent().removeClass("selected");
