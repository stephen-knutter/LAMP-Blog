        ...
        var iTime = new Date().getTime();
				switch(iscriptType){
			    	...
			    	case 'product':
						case 'front':
							type = iscriptType;
							if(iscriptType == 'product'){
								url = __LOCATION__ + '/ajax/ajax_prod_feed.php?'+iTime;
							} else {
      ...
      if(!gifStop){
        $.ajax({
          beforeSend: function(){
            if(!gifStop){
              gifStop = true;
              appendPane.append("<i class='fa fa-refresh fa-spin fa-3x fa-fw margin-bottom defaultspinner'></i>");
            }
          },
          type: 'POST',
          cache: 'false',
        	...
        	success: function(result){
			  if(result){
               ...
               gifStop = false;
							   ...
							   //VIDEO
 								...
 								$.each($result, function(i,v){
 									 ...
 									 if(j%2){
 											...
 											&& newVideo.video
 											&& newVideo.photo){
 												leftPane.append(doVideo(newVideo.video,newVideo.photo,timeStamp));
 												videojs('video-preview'+timeStamp,{},function(){});
 										 } else if(newVideo.photo){
 												leftPane.append("<div class='userPicWrap'>"+newVideo.photo+"</div>");
 										 }
 									 } else {
 											if(newVideo.vidtype == 'user'
 											&& newVideo.video
 											&& newVideo.photo){
 												 rightPane.append(doVideo(newVideo.video,newVideo.photo,timeStamp));
 												 videojs('video-preview'+timeStamp,{},function(){});
 											} else if(newVideo.photo) {
 												 rightPane.append("<div class='userPicWrap'>"+newVideo.photo+"</div>");
 											}
 									 }
 								 j++;
 								});
							...
							break;
							default:
							   appendPane.append($result.message);
							   gifStop = false;
							break;
						}
					 break;
